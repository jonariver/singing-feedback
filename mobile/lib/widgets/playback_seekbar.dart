...
///
/// Nur Slider + m:ss-Labels stecken in einem eng begrenzten
/// StreamBuilder<Duration>, NICHT das ganze Widget - Positions-Ticks (mehrmals
/// pro Sekunde waehrend der Wiedergabe) sollen keine breiteren Rebuilds
/// ausloesen. Waehrend eines aktiven Drags wird nur lokal (_dragValue)
/// aktualisiert, kein playFrom-Spam auf den geteilten Player - erst
/// onChangeEnd seekt tatsaechlich (gleiches _isBusy/Fehlermeldungs-Muster wie
...
class _PlaybackSeekbarState extends State<PlaybackSeekbar> {
  ...
  /// hier frisch per session.durationFor(bytes) abgefragt: SessionState.playFrom
  /// setzt _lastKnownDuration synchron auf Duration.zero zurueck, sobald es
  /// aufgerufen wird (noch bevor es zurueckkehrt) - ein zweiter Seek, der
  /// waehrend der erste noch haengt frisch nachfragen wuerde, saehe faelschlich
  /// schon 0 statt der tatsaechlichen Spurdauer.
  ///
